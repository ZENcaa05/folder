      ...
      if (window.firebaseDB) {
        ...
        onValue(ref(database, 'sensor/tegangan'), (snapshot) => {
          ...
          if (value !== null) {
            ...
            // Hitung persentase tegangan untuk progress bar (190-240V range)
            let vp = Math.floor(((value - 180) / 80) * 100);
            vp = Math.max(0, Math.min(100, vp));
            $('#current-bar').css('width', vp + '%');
          }
        });
        ...
        onValue(ref(database, 'sensor/arus'), (snapshot) => {
          const value = snapshot.val();
          if (value !== null) {
            $('#current-value').text(value.toFixed(1) + ' A');
            const cp = Math.floor((value / 10) * 100);
        ...
        onValue(ref(database, 'sensor/jarak'), (snapshot) => {
          ...
          if (sensorDistance !== null) {
            // Tampilkan nilai jarak mentah
            $('#distance-value').text(sensorDistance.toFixed(1));
            ...
            // Tampilkan tinggi air terkonversi
            $('#distance-water-height').text(waterHeight.toFixed(1));
        ...
        // ========== TOTAL PENYIRAMAN REAL-TIME ==========
        onValue(ref(database, 'statistik/totalPenyiraman'), (snapshot) => {
          const value = snapshot.val();
          if (value !== null) {
            totalWateringCount = value;
            $('#total-watering').text(totalWateringCount + ' Kali');
          }
        });
      }
